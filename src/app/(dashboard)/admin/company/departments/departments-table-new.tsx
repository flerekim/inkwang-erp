...
interface DepartmentsTableNewProps {
  data: Department[];
}

export function DepartmentsTableNew({ data }: DepartmentsTableNewProps) {
  ...
  const handleUpdateCell = React.useCallback(
    async (rowIndex: number, columnId: string, value: string) => {
      ...
      try {
        ...
        const result = await updateDepartment(department.id, updateData);
  ...
  const handleDeleteSelected = async () => {
    ...
    const selectedDepartments = selectedIndices.map((index) => tableData[index]).filter(Boolean);

    if (selectedDepartments.length === 0) {
      toast({
        variant: 'destructive',
        title: '선택 오류',
        description: '삭제할 부서를 선택해주세요.',
      });
      return;
    }
    ...
    try {
      ...
      const results = await Promise.allSettled(
        selectedDepartments.map((department) => deleteDepartment(department.id))
      );
      ...
      if (failures.length > 0) {
        ...
        toast({
          title: '삭제 완료',
          description: `${selectedDepartments.length}개의 부서가 삭제되었습니다.`,
        });
      }
  ...
  const displayData = React.useMemo(() => {
    if (newRowData) {
      return [newRowData as Department, ...tableData];
    }
    return tableData;
